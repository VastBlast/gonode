...
func GenAsyncReturnDoubleTypeCode(resultStructName string) string {
	_ = resultStructName
	code := ""
	if tools.CheckOS64Unit() {
		code = tools.FormatCodeIndentLn(`double wg_res_ = 0.0;
  if (!wg_is_error && wg_async_res != NULL) {
    double* wg_res_ptr_ = (double*)wg_async_res->data;
...
func GenAsyncCallbackArgDoubleTypeCode() string {
    ...
    if (!wg_is_error) {
      Number wg_double_ = Number::New(wg_env_scope, wg_res_);
      wg_result = wg_double_;
    }`
}
