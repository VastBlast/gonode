...
func GenAsyncReturnArrayTypeCode(resultStructName string) string {
	_ = resultStructName
...
func GenAsyncCallReturnArrayTypeCode(methodName string, argNames []string, cleanupLabel string, resultStructName string) string {
	code := `
  // -------- genHandlerCode
  ...
  if (wg_res_buf_ == NULL) {
    ...
    wg_send_async_error("alloc async array result");
    goto ` + cleanupLabel + `;
  }
  if (wg_src_len_ > 0) {
  ...
  if (wg_async_res_success == NULL) {
    free(wg_res_buf_);
    wg_send_async_error("alloc async array result wrapper");
    goto ` + cleanupLabel + `;
  }
...
func GenAsyncCallbackArgArrayTypeCode() string {
    ...
    if (!wg_is_error) {
      Array wg_arr_ = wg_string_to_array(wg_res_, wg_env_scope);
      wg_result = wg_arr_;
    }`
	return code
}
